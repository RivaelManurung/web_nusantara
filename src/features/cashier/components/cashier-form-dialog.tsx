...
export function CashierFormDialog({ open, onOpenChange, editing }: Props) {
  ...
  const resolver = useMemo(() => zodResolver(schemaFor(isEditing)), [isEditing]);
  ...
  /** Closing hides the password again, so reopening never reveals a typed one. */
  function handleOpenChange(next: boolean) {
    if (!next) setPasswordVisible(false);
    onOpenChange(next);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{isEditing ? "Ubah Kasir" : "Tambah Kasir"}</DialogTitle>
          <DialogDescription>
            Kasir dapat masuk ke aplikasi dan melayani transaksi di toko yang
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                aria-describedby={
                  errors.name ? "cashier-name-error" : undefined
                }
                {...register("name")}
              />
              {errors.name ? (
